...
final class Stack(val memory: Memory) extends AllocatorFactory {
  ...
  sealed trait StackAllocator extends AllocatorDefintion[Allocator] {
    def allocate[S[X <: AllocatorDefintion[X]] <: Struct[X]](implicit structDef: StructDef[S]): S[Allocator] = {
